...
class Representation():
    ...
    def load_patient_data(self, culture_filename, medication_filename):
        ...
        medication_dict, temp = data_reader.read_csv(medication_filename, RELEVANT_MO_LIST=self.RELEVANT_MO_LIST, data_type="dot")

        ab_length = len(self.ab_dict.keys())

        max_medication_value = np.ones(ab_length)
            ...
            for ab_index in range(len(self.ab_names)):
                ...
                for medication_name in medication_dict[patient_id].keys():
                    if ab_name in medication_name:
                        ...
                        dot = process_data.count_dot(medication_dict[patient_id][medication_name], limit=resistance_date)
                        medication_vector[self.ab_dict[ab_name]] = dot
                        if max_medication_value[self.ab_dict[ab_name]] < dot:
                            max_medication_value[self.ab_dict[ab_name]] = dot
